...
local function select_returns_selected_id()
  ...
  local label = picker_opts.items[1].label or ""
  local summary = string.format("%s|%s|%s", picker_opts.title or "", label, selected or "")
  assert.eq(summary, "Run Config|Android|ios", "run picker")
end
